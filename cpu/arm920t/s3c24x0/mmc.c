...
void PrvDelay (unsigned int usecs)
{
	udelay(usecs);
	udelay(usecs);
}
...
// SV - Added this to fix a bug where some cards couldn't be detected every 2nd reset.
// Scoping the signals revealed that all of the data lines were sitting at 1.2V during 
// reset due to CPU internal pull-ups.
void mmc_por(void)
{
	S3C24X0_GPIO * const gpioP = S3C24X0_GetBase_GPIO();
	
	// Set all SD lines output low	
	gpioP->GPEDAT &= ~((1 << 10) | (1 <<  9) | (1 <<  8) | (1 <<  7) | (1 <<  6) | (1 <<  5));
	gpioP->GPECON &= ~((3 << 20) | (3 << 18) | (3 << 16) | (3 << 14) | (3 << 12) | (3 << 10));
	gpioP->GPECON |=  ((1 << 20) | (1 << 18) | (1 << 16) | (1 << 14) | (1 << 12) | (1 << 10));
	...
	// Turn off power, SD_ENn high			
	gpioP->GPGDAT |= (1 << 1);
	
	PrvDelay(500000);
	
	// Turn on power, SD_ENn low		
	gpioP->GPGDAT &= ~(1 << 1);
	
	PrvDelay(500000);
	
	// Set all SD lines back to special function
	gpioP->GPECON &= ~((3 << 20) | (3 << 18) | (3 << 16) | (3 << 14) | (3 << 12) | (3 << 10));
	gpioP->GPECON |=  ((2 << 20) | (2 << 18) | (2 << 16) | (2 << 14) | (2 << 12) | (2 << 10));
	
	// SD_CDn back to input	
	gpioP->GPGCON &= ~(3 << 16);
...
// Turn on power	
	...
	sdi->SDIIMSK = 0x0;
	sdi->SDICON = 0x01; // enable clock
	PrvDelay(125000); /* FIXME: 74 SDCLK cycles */
